...
// Count total
if ($search) {
    $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM dosen WHERE nama LIKE ? OR nidn LIKE ?");
    $search_term = "%{$search}%";
...
// Count active programs/jurusan
$stmt_jurusan = $pdo->query("SELECT COUNT(DISTINCT jurusan) as count FROM dosen");
$total_jurusan = $stmt_jurusan->fetchColumn();

// Get data
if ($search) {
    $stmt = $pdo->prepare("
        SELECT nidn, nama, email, jurusan
        FROM dosen
        WHERE nama LIKE ? OR nidn LIKE ?
        ORDER BY nama
        LIMIT ? OFFSET ?
    ");
    $search_term = "%{$search}%";
            ...
            <main class="page-content">
                <div class="page-header">
                    <div class="page-header-left">
                        <div class="breadcrumb">ADMINISTRATOR <span>&gt; MANAJEMEN DOSEN</span></div>
                        <h1 class="page-title"><?= h($page_title) ?></h1>
                    </div>
                    <div class="page-header-right">
                        <a href="<?= APP_URL ?>/admin/dosen/tambah.php" class="btn-primary-custom">
                ...
                <div class="stats-grid">
                    <div class="stat-card">
                        ...
                        <div class="stat-content">
                            ...
                            <div class="stat-label">Total Dosen Pengajar</div>
                        </div>
                    </div>
                    <div class="stat-card indigo">
                        ...
                        <div class="stat-content">
                            ...
                            <div class="stat-label">Program Studi/Jurusan</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="table-actions">
                        <form method="GET" class="search-form">
                            <div class="search-input-wrapper">
                                ...
                                <input type="text" name="search" class="search-input" placeholder="Cari nama dosen atau NIDN..." value="<?= h($search) ?>">
                            </div>
                            <button type="submit" class="btn-search">Cari</button>
                            <?php if ($search): ?>
                                <a href="<?= APP_URL ?>/admin/dosen/index.php" class="btn-ghost">Reset</a>
                            <?php endif; ?>
                        </form>
                    </div>
                                ...
                                <tbody>
                                    <?php foreach ($dosen as $idx => $d): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <span class="dsn-name"><?= h($d['nama']) ?></span>
                                                <span class="dsn-nidn">NIDN: <?= h($d['nidn']) ?></span>
                                            </td>
                                            <td><a href="mailto:<?= h($d['email']) ?>" style="color: #4B5563; text-decoration: none;"><i class="bi bi-envelope" style="margin-right: 6px; color: #9CA3AF;"></i><?= h($d['email']) ?></a></td>
                                            <td><span class="badge-jurusan"><?= h($d['jurusan']) ?></span></td>
                                            <td>
                                                <div class="action-flex" style="justify-content: center;">
                                                    <a href="<?= APP_URL ?>/admin/dosen/edit.php?nidn=<?= urlencode($d['nidn']) ?>" class="btn-icon" title="Edit Data">
                                                        <i class="bi bi-pencil-square"></i>
                                                    </a>
                                                    <a href="<?= APP_URL ?>/admin/dosen/hapus.php?nidn=<?= urlencode($d['nidn']) ?>" class="btn-icon danger" title="Hapus Data" onclick="return confirm('Apakah Anda yakin ingin menghapus dosen ini?');">
