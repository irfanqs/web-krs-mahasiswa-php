...
<div style="display:flex;align-items:center;gap:16px;margin-bottom:24px;">
    ...
    <h1 style="font-size:28px;font-weight:700;color:#0B1E4F;margin:0;">Edit Semester</h1>
</div>
<div style="background:white;border-radius:16px;padding:28px;box-shadow:0 2px 10px rgba(11,30,79,.06);max-width:600px;">
    <form method="POST" action="{{ route('admin.semester.update', $semester->id_semester) }}">
        @csrf @method('PUT')
        <div style="margin-bottom:16px;">
            ...
            <input type="text" name="tahun_ajaran" value="{{ old('tahun_ajaran', $semester->tahun_ajaran) }}" placeholder="e.g. 2023/2024"
        ...
        <div style="margin-bottom:16px;">
            ...
            <select name="tingkatan_semester" style="width:100%;padding:12px 14px;border:1px solid #E4E7EE;border-radius:10px;font-family:inherit;font-size:14px;color:#0B1E4F;">
                @foreach(['ganjil','genap'] as $t)
                <option value="{{ $t }}" {{ old('tingkatan_semester',$semester->tingkatan_semester)===$t?'selected':'' }}>{{ ucfirst($t) }}</option>
                @endforeach
            </select>
