...
<div style="display:flex;align-items:center;gap:16px;margin-bottom:24px;">
    ...
    <h1 style="font-size:28px;font-weight:700;color:#0B1E4F;margin:0;">Edit Jadwal Kuliah</h1>
</div>
    ...
    <form method="POST" action="{{ route('admin.jadwal.update', $jadwal->id_jadwal) }}">
        ...
        <div style="margin-bottom:16px;">
            ...
            <select name="id_matkul" style="width:100%;padding:12px 14px;border:1px solid {{ $errors->has('id_matkul')?'#E04F5F':'#E4E7EE' }};border-radius:10px;font-family:inherit;font-size:14px;color:#0B1E4F;">
                ...
                <option value="{{ $m->id_matkul }}" {{ old('id_matkul',$jadwal->id_matkul)==$m->id_matkul?'selected':'' }}>{{ $m->kode_matkul }} — {{ $m->nama_matkul }}</option>
                @endforeach
            </select>
        ...
        <div style="margin-bottom:16px;">
            ...
            <select name="id_dosen" style="width:100%;padding:12px 14px;border:1px solid {{ $errors->has('id_dosen')?'#E04F5F':'#E4E7EE' }};border-radius:10px;font-family:inherit;font-size:14px;color:#0B1E4F;">
                ...
                <option value="{{ $d->nidn }}" {{ old('id_dosen',$jadwal->id_dosen)===$d->nidn?'selected':'' }}>{{ $d->nama }}</option>
                @endforeach
            </select>
        ...
        <div style="margin-bottom:16px;">
            ...
            <select name="id_semester" style="width:100%;padding:12px 14px;border:1px solid {{ $errors->has('id_semester')?'#E04F5F':'#E4E7EE' }};border-radius:10px;font-family:inherit;font-size:14px;color:#0B1E4F;">
                ...
                <option value="{{ $s->id_semester }}" {{ old('id_semester',$jadwal->id_semester)==$s->id_semester?'selected':'' }}>{{ $s->tahun_ajaran }} {{ ucfirst($s->tingkatan_semester) }} {{ $s->status==='aktif'?'(Aktif)':'' }}</option>
                @endforeach
            </select>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px;">
            <div>
                ...
                <input type="text" name="ruang" value="{{ old('ruang', $jadwal->ruang) }}" placeholder="e.g. R.402"
