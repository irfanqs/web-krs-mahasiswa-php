...
<div style="display:flex;align-items:center;gap:16px;margin-bottom:24px;">
    ...
    <h1 style="font-size:28px;font-weight:700;color:#0B1E4F;margin:0;">Edit Dosen</h1>
</div>
<div style="background:white;border-radius:16px;padding:28px;box-shadow:0 2px 10px rgba(11,30,79,.06);max-width:600px;">
    <form method="POST" action="{{ route('admin.dosen.update', $dosen->nidn) }}">
        @csrf @method('PUT')
        @foreach(['nama'=>'Nama Lengkap','email'=>'Email','jurusan'=>'Jurusan'] as $field => $label)
        <div style="margin-bottom:16px;">
            ...
            <input type="{{ $field==='email'?'email':'text' }}" name="{{ $field }}" value="{{ old($field,$dosen->$field) }}"
                style="width:100%;padding:12px 14px;border:1px solid #E4E7EE;border-radius:10px;font-family:inherit;font-size:14px;color:#0B1E4F;">
