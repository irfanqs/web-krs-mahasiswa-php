...
<div style="display:flex;align-items:center;gap:16px;margin-bottom:24px;">
    ...
    <h1 style="font-size:28px;font-weight:700;color:#0B1E4F;margin:0;">Tambah Mahasiswa</h1>
</div>
...
<div style="background:white;border-radius:16px;padding:28px;box-shadow:0 2px 10px rgba(11,30,79,.06);max-width:600px;">
    <form method="POST" action="{{ route('admin.mahasiswa.store') }}">
        @csrf
        @foreach(['nim'=>['NIM','text','required','e.g. 21010023'],'nama'=>['Nama Lengkap','text','required',''],'email'=>['Email','email','required',''],'password'=>['Password','password','required','min. 6 karakter'],'angkatan'=>['Angkatan','number','required','2021'],'program_studi'=>['Program Studi','text','required','']] as $field => [$label,$type,$req,$placeholder])
        <div style="margin-bottom:16px;">
            ...
            <input type="{{ $type }}" name="{{ $field }}" value="{{ old($field) }}" placeholder="{{ $placeholder }}" {{ $req }}
                style="width:100%;padding:12px 14px;border:1px solid #E4E7EE;border-radius:10px;font-family:inherit;font-size:14px;color:#0B1E4F;{{ $errors->has($field)?'border-color:#E04F5F;':'' }}">
